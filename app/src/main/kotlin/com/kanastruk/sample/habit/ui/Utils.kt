...
fun SharedPreferences.getCredentials(): Credentials? {
    val idToken = getString(FirebaseConfig.PrefKey.ID_TOKEN.name, null)
    val localId = getString(FirebaseConfig.PrefKey.LOCAL_ID.name, null)
    val refreshToken = getString(FirebaseConfig.PrefKey.REFRESH_TOKEN.name, null)
...
fun SharedPreferences.setCredentials(credentials: Credentials) {
    with(edit()) {
        putString(FirebaseConfig.PrefKey.ID_TOKEN.name,credentials.idToken)
        putString(FirebaseConfig.PrefKey.LOCAL_ID.name,credentials.localId)
        putString(FirebaseConfig.PrefKey.REFRESH_TOKEN.name,credentials.refreshToken)
        apply()
    }
}
