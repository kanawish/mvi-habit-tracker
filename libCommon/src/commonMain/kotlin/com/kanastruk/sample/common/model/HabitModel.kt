 ...
 *
 * class HabitModel {
 *  + habitsStore:StateFlow<HabitState>
 *  + process( <i>Intent<HabitState></i> )
 * }
 * @enduml
 *
 * @startuml
 ...
 * class HabitModel {
 *  + habitsStore:StateFlow<HabitState>
 *  + errors:StateFlow<HabitError>
 *  - process( <i>Intent<HabitState></i> )
 *  + processReloadIntent( )
 *  + processAddEntryIntent( <i>String,Int</i> )
 *  + processUndoEntryIntent( <i>String</i> )
 *  + processCreateHabitIntent( <i>Habit</i> )
 *  + processUpdateHabitIntent( <i>String, Habit</i> )
 *  + processArchiveHabitIntent( <i>String</i> )
 * }
 * @enduml
 */
class HabitModel(
    authModel: AuthModel,
    val httpClientBuilder: (authToken: String) -> HttpClient,
    scope: CoroutineScope,
) : HabitApi, CoroutineScope by scope {
    private val log: Logger = Logger.withTag("HabitModel")
    ...
    init {
        val apiFlow = authModel.authStore
            ...
            .map { nullableCreds ->
                nullableCreds?.let { creds ->
                    InternalState(creds, httpClientBuilder(creds.idToken))
                }
            }
        ...
        apiFlowJob = apiFlow
            ...
            .onCompletion { log.d("apiFlowJob completed $it") }
            .launchIn(this)
    ...
    private fun authedProcess(
        ...
        currentHabitApi.value?.let { internalState ->
            if (predicate(currentHabitState)) {
                ...
                log.w("authedProcess() call ignored: predicate returned `false` for $currentHabitState.")
            }
        } ?: log.w("authedProcess() call ignored: internal state is null.")
    }
    ...
    fun processReloadIntent() {
        authedProcess(::isReloadable) { (credentials, client), oldState ->
            val userId = credentials.localId
            launch { client.reloadProcess(userId) }
    ...
    fun processAddEntryIntent(habitId: String, quantity: Int) {
        authedProcess(::isModifiable) { (credentials, client), oldState ->
            launch {
     ...
     *
     * TODO: Should 'scope' removals to the current day. Unbounded for now.
     */
            ...
            launch {
                oldState.entries[habitId]
                    ...
                    ?: process { old -> old.copy(cacheState = MODIFIED) }
                        .also { log.d("⚠️  No entries left for this habit.") }
            }
            oldState.copy(cacheState = BUSY).touch()
        }
    }
    ...
    fun processCreateHabitIntent(habit: Habit) {
        authedProcess(::isModifiable) { (credentials, api), oldState ->
            launch { api.createHabit(credentials.localId, habit) }
            oldState.copy(cacheState = BUSY).touch()
        }
    }
    ...
    fun processUpdateHabitIntent(habitId: String, habit: Habit) {
        authedProcess(::isModifiable) { (credentials, api), oldState ->
            launch { api.updateHabit(credentials.localId, habitId, habit) }
            oldState.copy(cacheState = BUSY).touch()
        }
    }
    ...
    fun processArchiveHabitIntent(habitId: String) {
        ...
        } ?: log.w("processArchiveHabitIntent() call ignored: could not find habit '$habitId'.")
        // NOTE: Crash-worthy?
    }
     ...
     */
    private suspend fun HttpClient.reloadProcess(userId: String) {
        ...
        if (habits.isSuccessful) {
            ...
            if (entries.isSuccessful) {
                val entriesBody = entries.successBody ?: emptyMap()
                val habitsBody = habits.successBody ?: emptyMap()
                process { old ->
                    old.copy(
                        cacheState = LOADED,
                        habits = habitsBody,
     ...
     */
    private suspend fun HttpClient.createHabit(userId: String, habit: Habit) {
        val response = postHabit(userId, habit)
        if (response.isSuccessful) {
            response.successBody?.let { nameResponse ->
                process { old ->
                    old.copy(
                        cacheState = MODIFIED,
     ...
     */
    private suspend fun HttpClient.updateHabit(userId: String, habitId: String, habit: Habit) {
        val response = putHabit(userId, habitId, habit)
        if (response.isSuccessful) {
            response.successBody?.let { savedHabit ->
                process { old ->
                    old.copy(
                        cacheState = MODIFIED,
     ...
     */
    private suspend fun HttpClient.createEntry(userId: String, habitId: String, entry: Entry) {
        val response = postEntry(userId, habitId, entry)
        if (response.isSuccessful) {
            response.successBody?.name?.let { newEntryId ->
                process { old ->
    ...
    private suspend fun HttpClient.removeEntry(userId: String, habitId: String, entryId: String) {
